...
function PuzzleBuilder() {
  ...
  // ─── Save to Supabase ────────────────────────────────────────────────
  const openSaveModal = () => {
    setDraftTitle(puzzleTitle);
    setShowSaveModal(true);
  };

  const handleSaveToCloud = async () => {
    setShowSaveModal(false);
    const titleToSave = draftTitle.trim() || 'Untitled Puzzle';
    setPuzzleTitle(titleToSave);
    setSaving(true);
    setError(null);
    try {
      const record = await savePuzzle(titleToSave, pageSize, entries, puzzleDbId ?? undefined);
      setPuzzleDbId(record.id);
      setSaveSuccess(true);
      ...
      {showSaveModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            <h2 className="text-lg font-semibold text-gray-900 mb-1">Save Puzzle</h2>
            <p className="text-sm text-gray-500 mb-4">Give your puzzle a name before saving to the cloud.</p>
            <label className="block text-sm font-medium text-gray-700 mb-1">Puzzle name</label>
            <input
              autoFocus
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 focus:ring-2 focus:ring-blue-400 outline-none mb-5"
              value={draftTitle}
              onChange={(e) => setDraftTitle(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') handleSaveToCloud(); if (e.key === 'Escape') setShowSaveModal(false); }}
